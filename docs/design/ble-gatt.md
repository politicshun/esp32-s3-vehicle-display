...
| 5 | dtc_code | uint8 | [0\|255] | 고장 코드(단일 열거값, 비트마스크 아님). **0=정상, 1~255=고장(포괄) — 세부 코드별 의미 분류는 없음(2026-08-03 사용자 확인).** `docs/hardware/cluster.dbc`에 VAL_ 테이블이 없고, `main/ui/ui.c`도 raw 값을 그대로 hex로만 보여줄 뿐 해석하지 않는 것과 일치. 앱도 "0=정상 / nonzero=고장(코드값은 raw hex로만 표시)" 이상으로 해석하지 말 것 — 세부 분류가 생기면 인버터팀 스펙 확정 후 이 표를 갱신 |
